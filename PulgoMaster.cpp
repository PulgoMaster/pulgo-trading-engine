...
void PulgoMaster::dateFilter() {
    ...
    int dateOption;
    do {
        cout << "Filter by oldest or earliest date\n";
        cout << "0: Oldest (latest timestamp)\n";
        cout << "1: Earliest (first timestamp)\n";
        cin >> dateOption;

        if (dateOption == 0 || dateOption == 1) {
            Orderbook result = orderBookList[0];

            for (const auto& o : orderBookList) {
                if (dateOption == 0) { // oldest = latest date
                    if (o.dateTime > result.dateTime)
                        result = o;
                } else { // earliest = earliest date
                    if (o.dateTime < result.dateTime)
                        result = o;
                }
            }

            cout << "The " << (dateOption == 0 ? "OLDEST" : "EARLIEST")
                << " entry is:\n";
            cout << result.dateTime << " | "
                << toString(result.bidAsk) << " | "
                << toStringProduct(result.product)
                << " | Price: " << fixed << setprecision(8) << result.buyCost
                << " | Amount: " << result.coinAmmount << "\n\n";
        } else {
            cout << "Invalid — choose 0 or 1\n";
        }
    } while (dateOption != 0 && dateOption != 1);
}
